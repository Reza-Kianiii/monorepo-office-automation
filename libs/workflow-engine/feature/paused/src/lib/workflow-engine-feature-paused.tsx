...
import { useGetPausedQuery } from '@office-automation/workflow-engine/data/data-paused';

export function WorkFlowEngineFeaturePaused() {
  ...
  const selectedWorkflowEnginePaused = useRef<any>(null);
  ...
  return (
    <div className=" flex flex-1 flex-col  h-full ">
      ...
      <div className="flex flex-1 flex-col h-full">
        <Box className="mt-2 flex-[1_1_0] overflow-auto">
          <DataGridPremium
            ...
            apiRef={gridApiRef}
            loading={isLoadingPaused}
            slotProps={{
              row: {
                onFocus: (event: any) => {
                  const rowId = event.currentTarget.getAttribute('data-id');
                  const row = gridApiRef.current?.getRow(rowId ?? 0);
                  selectedWorkflowEnginePaused.current = row;
                },
              },
            }}
            checkboxSelection
      ...
      {operation === 'PROSECCMAKER' && (
        <WorkFlowEngineFeatureSharedDialogProcessMaker
          data={selectedWorkflowEnginePaused.current}
      ...
      {operation === 'INCOMINGDOCUMENTS' && (
        <WorkFlowEngineFeatureSharedDialogIncomingDocument
          id={selectedWorkflowEnginePaused.current?.app_uid}
          onclose={() => setOperation('null')}
        />
      )}
...
export default WorkFlowEngineFeaturePaused;
